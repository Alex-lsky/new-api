...
func UpdateOption(c *gin.Context) {
	...
	case tool_hosting.ProvidersOptionKey:
		err = tool_hosting.ValidateToolHostingProvidersJSON(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": err.Error(),
	...
	case tool_hosting.BindingsOptionKey:
		err = tool_hosting.ValidateToolHostingBindingsJSON(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": err.Error(),
	...
	case "ModelPricingAlias":
		err = price_alias.UpdatePriceAliasByJSONString(option.Value.(string))
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "模型定价别名设置失败: " + err.Error(),
			})
			return
		}
